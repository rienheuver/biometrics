...
hold on;
plot(FNMR(1,:),FNMR(2,:));
title('FMR and FNMR for different thresholds');
xlabel('Threshold');
ylabel('Scores');
figure;
plot(DET(:,1),(DET(:,2)));
...
xlabel('FMR');
ylabel('FNMR');
%Lets see about the logs
figure;
plot(DET(:,1),log(DET(:,2)));
title('log in Y');
xlabel('FMR');
ylabel('FNMR');
figure;
plot(log(DET(:,1)),DET(:,2));
title('log in X');
xlabel('FMR');
ylabel('FNMR');
figure;
plot(log(DET(:,1)),log(DET(:,2)));
title('log in X and Y');
xlabel('FMR');
ylabel('FNMR');
%plot the ROC
figure;
